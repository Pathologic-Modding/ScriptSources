...
var object g_fire, g_smoke, g_grenade;
...
maintask TGrenade
{
	void OnCollision(object who, object actor, Vector vPos, Vector vNormal) {
		...
		while (victims->Next(bCollide, victim), bCollide) {
			...
			if (victim != Owner) {
				@ReportAttack(victim, Owner);
				@ReportHit(victim, Owner, c_iHitTypeOscoloc, fDamage, fGrenageDamage);
				@BroadcastPlayerDamage(victim, scene, Owner);
			}
		}
		victims = null;
	...
	void init(void) {
		object body;
		@CreateRigidBody(body, [0, 0, 0], 15, StartVelocity, [2, 2, 2], Owner);
		body->Enable(true);
		g_grenade = body;
